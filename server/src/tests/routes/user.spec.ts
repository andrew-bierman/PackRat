...
describe('User Routes', () => {
    ...
    describe('resetPassword', () => {
        it('should reset user password', async () => {
            ...
            const resetUrl = await userClass.generateResetToken(env.JWT_SECRET, '', testUser.id)
            const resetToken = resetUrl.split('token=').pop()

            const response = await caller.resetPassword({ resetToken: resetToken, password: newPassword })
    ...
    describe('editUser', () => {
        it('should update user fields', async () => {
            ...
            const [editedUser] = await caller.editUser({ id: testUser.id, name: updatedFields.name, email: updatedFields.email })
