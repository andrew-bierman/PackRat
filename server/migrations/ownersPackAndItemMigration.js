...
const main = async () => {
    ...
    const itemsToUpdate = await Item.find({ packs: { $exists: true, $not: { $size: 0 } }, owners: { $exists: false } });
    ...
    const packsToUpdate = await Pack.find({ $or: [{ owners: { $exists: false } }, { owners: { $size: 0 } }] });
