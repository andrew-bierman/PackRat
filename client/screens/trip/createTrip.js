...
export default function Trips() {
  ...
  console.log("🚀 ~ file: createTrip.js:41 ~ Trips ~ selectedSearch:", selectedSearch)
  const { data: weatherData, isLoading: weatherLoading, isError: weatherError } = useFetchWeather(latLng);
  const { data: weatherWeekData, isLoading: weekWeatherLoading, isError: weekWeatherError } = useFetchWeatherWeak(latLng)
  const {
    data: parks,
    error: parksError,
    ...
    <ScrollView nestedScrollEnabled={true}>
      <RStack style={styles.mutualStyles}>
        ...
        <RStack style={styles.container}>
          ...
          />
          {
            !weekWeatherError && !weatherError && !weatherLoading && !weekWeatherLoading &&
            <WeatherCard
