    ...
    fs.readFile(filePath, 'utf8', function (err, data) {
      ...
      fs.writeFile(filePath, result, 'utf8', function (err) {
        if (err) {
          ...
          console.log(`Successfully replaced ${replaceToken} in ${filePath} with ${newValue}`);
        }
      });
    });
  }
...
replaceInFile(podfilePath, 'MAPBOX_DOWNLOADS_TOKEN_FROM_ENV', process.env.MAPBOX_DOWNLOADS_TOKEN);
replaceInFile(gradlefilePath, 'MAPBOX_DOWNLOADS_TOKEN_FROM_ENV', process.env.MAPBOX_DOWNLOADS_TOKEN);
replaceInFile(appJsonPath, 'MAPBOX_DOWNLOADS_TOKEN_FROM_ENV', process.env.MAPBOX_DOWNLOADS_TOKEN);
