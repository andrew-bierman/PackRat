          ...
          user?.id === data.owner_id && (
            <ThreeDotsMenu onOpenChange={handleActionsOpenChange}>
              <YStack gap="$1">
                ...
                <RButton onPress={() => setIsCopyPackModalOpen(true)}>Copy Pack</RButton>
               </YStack>
            </ThreeDotsMenu>
          )
        }
      />
      <CopyPackModal currentPack={data} isOpen={isCopyPackModalOpen} onSuccess={() => setIsCopyPackModalOpen(false)} />
    </>
  );
};
