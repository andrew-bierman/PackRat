...
const useTableLayout = () => {
  const [flexArr, setFlexArr] = useState([2, 1, 1, 1, 0.65, 0.65, 0.65]);
  ...
  useEffect(() => {
    if (
      ...
      setFlexArr([1, 1, 1, 1]);
      setHeading(['Item Name', 'Weight', 'Quantity', 'Options']);
    }
  }, []);

  return { flexArr, heading };
};
...
const TableContainer = ({
  ...
  const { flexArr, heading } = useTableLayout();
